...
// 如果发送成功, 返回1, 否则返回-1.
int son_sendpkt(int nextNodeID, sip_pkt_t* pkt, int son_conn) { return 0; }
...
int son_recvpkt(sip_pkt_t* pkt, int son_conn) { return 0; }
...
int getpktToSend(sip_pkt_t* pkt, int* nextNode, int sip_conn) { return 0; }
...
// PKTSTOP1 -- 接收到'!', 期待'#'以结束数据的接收
// 如果成功接收报文, 返回1, 否则返回-1.
int recvpkt(sip_pkt_t* pkt, int conn) { return 0; }
